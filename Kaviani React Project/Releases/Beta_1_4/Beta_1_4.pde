...
public void setup()
{
  ...
  // alRBlooms.add(new BD_Bloom(beat, 1.3, 75, 82, 124, 150, 0, 50, 0, 255, fDAlpha, fDAlpha, -width, width, -height, height)); // THIS MUST BE UNCOMMENTED FOR ATTEMPT 4
  //RBloom = new BD_Bloom (beat, 1.3, 75, 82, 124, 150, 0, 50, 0, 255, fDAlpha, fDAlpha, -width, width, -height, height);
}
...
{ 
  ...
  //    //println(i);                       // I know exactly what I am trying to do, just not how to execute it              // Attempt 1
  ...
  //  fDAlpha *= fDR;
  //  RBloom.run();
  //  if (fDAlpha < 1) {                   // Attempt 4 // PROGRESSSSSS!!!!
  //    alRBlooms.remove(RBloom);         // Only one RBloom instance in the array though... // Looks like it stays on screen and gets removed after a certain amount of time
  //    fDAlpha = 100;                   // It doesnt seem to actually bloom though
  //    alRBlooms.add(new BD_Bloom(beat, 1.3, 75, 82, 124, 150, 0, 50, 0, 255, fDAlpha, fDAlpha, -width, width, -height, height));
  //  }                                // for some reason this does some funky stuff with the Bloom 1 and 2 that already work                                                                  

  //  //println(fDAlpha);
  //  println(alRBlooms.size());
  //}
  
  if (alRBlooms.size() < 15){
  alRBlooms.add(new BD_Bloom(beat, 1.3, 75, 82, 124, 150, 0, 50, 0, 255, fDAlpha, fDAlpha, -width, width, -height, height));
  for (int i = 0; i <alRBlooms.size(); i++) {
    BD_Bloom RBloom = (BD_Bloom) alRBlooms.get(i);
    fDR = 0.95;
    ...
    if (fDAlpha < 1) {                  // Attempt 5 // Array size is limited however they only spawn for one cycle...
      alRBlooms.remove(RBloom);       
      fDAlpha = 100;                
      //alRBlooms.add(new BD_Bloom(beat, 1.3, 75, 82, 124, 150, 0, 50, 0, 255, fDAlpha, fDAlpha, -width, width, -height, height)); // uncommenting this completley voidss the spawn
    }
    RBloom.run();
    //println(fDAlpha);
    println(alRBlooms.size());
